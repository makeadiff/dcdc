<template>
    <div class="response">
        <template v-if="response_type === 'choice'">
            <div class="choice" v-for="choice in choices" v-bind:key="choice.id" v-bind:choice="choice">
                <label>
                    <input type="radio" v-bind:value="choice.id" v-bind:key="choice.id" v-model="answer" />
                    {{ choice.name }}
                </label>
            </div>
        </template>
        <template v-else-if="this.response_type === 'longtext'">
            <textarea class="form-control" v-bind:key="question.id" v-model="answer"></textarea>
        </template>
        <template v-else-if="this.response_type === 'text'">
            <input type="text" class="form-control" v-bind:key="question.id" v-model="answer" />
        </template> 
